...
const PERSISTENCE_MODE = DATABASE_URL ? 'postgres' : 'sqlite'
...
function resolveDatabaseUrl() {
  ...
  const host = process.env.PGHOST || process.env.POSTGRES_HOST || process.env.RDS_HOSTNAME
  const database = process.env.PGDATABASE || process.env.POSTGRES_DB || process.env.RDS_DB_NAME || process.env.DB_NAME
  const user = process.env.PGUSER || process.env.POSTGRES_USER || process.env.RDS_USERNAME || process.env.DB_USER
  const password =
    process.env.PGPASSWORD || process.env.POSTGRES_PASSWORD || process.env.RDS_PASSWORD || process.env.DB_PASSWORD
  const port = process.env.PGPORT || process.env.POSTGRES_PORT || process.env.RDS_PORT || process.env.DB_PORT || 5432
  const protocol = (process.env.POSTGRES_PROTOCOL || 'postgresql').replace(/:\s*$/g, '').replace(/\s+/g, '')
...
function getPersistenceDiagnostics() {
  return {
    ...
    sqlitePath: PERSISTENCE_MODE === 'sqlite' ? SQLITE_DB_PATH : null,
    hasDatabaseUrl: Boolean(DATABASE_URL),
    databaseUrlPrefix: DATABASE_URL ? redactConnectionString(DATABASE_URL).slice(0, 80) : '',
    sslMode: DATABASE_URL ? DATABASE_SSL_MODE : null,
  }
}

module.exports = { init, getPersistenceDiagnostics }
